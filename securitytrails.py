...
class SecurityTrails:
    ...
    def test_connect(self):
        ...
        if r.status_code != 200:
            logger.warning(
                "get_domain:Error with query to Security Trails, error message: {}".format(output['message']))
            return False
        else:
            return True
    ...
    def get_subdomain(self, domain):
        ...
        if r.status_code != 200:
            logger.warning(
                "get_subdomain:Error with query to Security Trails, error message: {}".format(output['message']))
            return False
        else:
            return output
    ...
    def get_history_dns(self, domain, record_type):
        ...
        if record_type in type_check:
            ...
            if r.status_code != 200:
                ...
                output = r.json()
                logger.warning(
                    "get_history_dns:Error with query to Security Trails, error message: {}".format(output['message']))
                return False
            else:
                # Output results to json
                output = r.json()
            ...
            logger.warning(
                "get_history_dns: Invalid type, valid types are {}.  Error message received from server: {}".format(
                    str(", ".join(type_check)), output['message']))
            return False
    ...
    def get_history_whois(self, domain):
        ...
        if r.status_code != 200:
            ...
            output = r.json()
            logger.warning(
                "get_history_whois:Error with query to Security Trails, error message: {}".format(output['message']))
            return False
        else:
            # Output results to json
            output = r.json()
    ...
    def ip_explorer(self, ip):
        ...
        """
        # Returns the neighbors in any given IP level range and essentially allows you to explore closeby IP
        # addresses. This works by providing a specific IP address (or by starting at the whole unzoomed level).
        # Explorer will always yield IPs that will be grouped in blocks, so the output will always be 16.
        ...
        if r.status_code != 200:
            ...
            output = r.json()
            logger.warning(
                "ip_explorer:Error with query to Security Trails, error message: {}".format(output['message']))
            return False
        else:
            # Output results to json
            output = r.json()
    ...
    def domain_searcher(self, **kwargs):
        ...
        """
        # Filter and search specific records using this endpoint. Using simple filter composition, any type of data
        # fetching is possible. The post object uses a very simple dsl where the json key represents the type to
        # filter on and the value. Given this, you can create any number of queries, depending on the need. It's
        # worth noting that all of the filters are combined using AND fashion and work in combination.
        ...
        valid_filter = ["ipv4", "ipv6", "mx", "ns", "cname", "subdomain", "apex_domain", "soa_email", "tld",
                        "whois_email", "whois_street1", "whois_street2", "whois_street3", "whois_street4",
                        "whois_telephone", "whois_postalCode", "whois_organization", "whois_name", "whois_fax",
                        "whois_city", "keyword"]

        for key, value in kwargs.iteritems():
            if key not in valid_filter:
                logger.warning(
                    "domain_searcher:Error with query to Security Trails.  {} is not a valid filter. Ignoring this "
                    "key.  Valid formats are: {}".format(
                        str(key), str(", ".join(valid_filter))))
        ...
        if r.status_code != 200:
            ...
            output = r.json()
            logger.warning(
                "domain_searcher:Error with query to Security Trails, error message: {}".format(output['message']))
            return False
        else:
            # Output results to json
            output = r.json()
    ...
    def domain_searcher_stats(self, **kwargs):
        """
        :param List : Required - The type of information you want to filter on ipv4, ipv6, mx, ns, cname, subdomain,
        apex_domain, soa_email, tld, whois_email, whois_street1, whois_street2, whois_street3, whois_street4,
        whois_telephone, whois_postalCode, whois_organization, whois_name, whois_fax, whois_city, keyword
        ...
        """
        # By appending /stats at the end of the search URL, instead of getting the usual records, a stats object is
        # given in the response. This object contains some useful information like:tld count, hostname count,
        # domain count
        ...
        for key, value in kwargs.iteritems():
            if key not in valid_filter:
                logger.warning(
                    "domain_searcher_stats:Error with query to Security Trails.  {} is not a valid filter. Ignoring "
                    "this key.  Valid formats are: {}".format(
                        str(key), str(", ".join(valid_filter))))
        ...
        if r.status_code != 200:
            # Output results to json
            output = r.json()
            logger.warning("domain_searcher_stats:Error with query to Security Trails, error message: {}".format(
                output['message']))
            return False
        else:
            # Output results to json
